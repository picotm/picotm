...
find_by_id(struct fildes_sockbuftab* sockbuftab, const struct filebuf_id* id)
{
    ...
    while (sockbuf_beg < sockbuf_end) {
        ...
        const int cmp = filebuf_ref_if_id(&sockbuf_beg->base, id);
        if (!cmp) {
            return sockbuf_beg;
        }
             ...
             int fildes, struct picotm_error* error)
{
    ...
    while (sockbuf_beg < sockbuf_end) {
        ...
        const int cmp = filebuf_ref_or_set_up_if_id(&sockbuf_beg->base,
                                                    fildes, id, error);
        if (!cmp) {
            if (picotm_error_is_set(error)) {
                return NULL;
                             ...
                             struct picotm_error* error)
{
    ...
    filebuf_ref_or_set_up(&sockbuf->base, fildes, error);
    if (picotm_error_is_set(error)) {
