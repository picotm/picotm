...
{
    struct seekbuf_tx* seekbuf_tx = malloc(sizeof(*seekbuf_tx));
    if (!seekbuf_tx) {
        picotm_error_set_errno(error, errno);
        return nullptr;
    }
    seekbuf_tx_init(seekbuf_tx);
    return seekbuf_tx;
}

static void
seekbuf_tx_destroy(struct seekbuf_tx* self)
{
    seekbuf_tx_uninit(self);
    free(self);
}

static struct sockbuf_tx*
sockbuf_tx_create(struct picotm_error* error)
{
    struct sockbuf_tx* sockbuf_tx = malloc(sizeof(*sockbuf_tx));
    if (!sockbuf_tx) {
        picotm_error_set_errno(error, errno);
        return nullptr;
    }
    sockbuf_tx_init(sockbuf_tx);
    return sockbuf_tx;
}

static void
sockbuf_tx_destroy(struct sockbuf_tx* self)
{
    sockbuf_tx_uninit(self);
    free(self);
}
               ...
               struct fildes_log* log)
{
    ...
    picotm_slist_init_head(&self->sockbuf_tx_alloced_list);
    picotm_slist_init_head(&self->sockbuf_tx_active_list);

    self->openoptab = nullptr;
    self->openoptablen = 0;

    self->pipeoptab = nullptr;
    self->pipeoptablen = 0;
}

static void
cleanup_sockbuf_tx_alloced_list_cb(struct picotm_slist* item)
{
    struct sockbuf_tx* sockbuf_tx = sockbuf_tx_of_list_entry(item);
    sockbuf_tx_destroy(sockbuf_tx);
}

static void
cleanup_seekbuf_tx_alloced_list_cb(struct picotm_slist* item)
{
...
{
    /* Uninit allocated instances of |struct sockbuf_tx| */
    picotm_slist_cleanup_0(&self->sockbuf_tx_alloced_list,
                           cleanup_sockbuf_tx_alloced_list_cb);
    picotm_slist_uninit_head(&self->sockbuf_tx_alloced_list);
    picotm_slist_uninit_head(&self->sockbuf_tx_active_list);

    /* Uninit allocated instances of |struct seekbuf_tx| */
    picotm_slist_cleanup_0(&self->seekbuf_tx_alloced_list,
...
static bool
true_if_eq_seekbuf_id_cb(const struct picotm_slist* item, void* data)
{
    const struct seekbuf_tx* seekbuf_tx =
        seekbuf_tx_of_list_entry((struct picotm_slist*)item);
    assert(seekbuf_tx);
    assert(seekbuf_tx->seekbuf);
...
static struct seekbuf_tx*
get_seekbuf_tx(struct fildes_tx* self, const struct filebuf_id* id,
               struct picotm_error* error)
{
    /* Let's see if we already have the seekbuf in use.*/

    struct picotm_slist* pos = picotm_slist_find_1(&self->seekbuf_tx_active_list,
                                                   true_if_eq_seekbuf_id_cb,
                                                   (void*)id);
    if (pos != picotm_slist_end(&self->seekbuf_tx_active_list)) {
        struct seekbuf_tx* seekbuf_tx = seekbuf_tx_of_list_entry(pos);
        return seekbuf_tx;
    }
    ...
    pos = picotm_slist_front(&self->seekbuf_tx_alloced_list);
    if (pos) {
        picotm_slist_dequeue_front(&self->seekbuf_tx_alloced_list);
        return seekbuf_tx_of_list_entry(pos);
    }

    struct seekbuf_tx* seekbuf_tx = seekbuf_tx_create(error);
    if (picotm_error_is_set(error)) {
        return nullptr;
    }
                        ...
                        struct picotm_error* error)
{
    ...
    struct seekbuf_tx* seekbuf_tx = get_seekbuf_tx(self, &id, error);
    if (picotm_error_is_set(error)) {
        goto err_get_seekbuf_tx;
    }

    if (seekbuf_tx_holds_ref(seekbuf_tx)) {
        return seekbuf_tx; /* fast path: already holds a reference */
    }
    ...
    seekbuf_tx_ref_or_set_up(seekbuf_tx, seekbuf, error);
    if (picotm_error_is_set(error)) {
        goto err_seekbuf_tx_ref_or_set_up;
    }

    picotm_slist_enqueue_front(&self->seekbuf_tx_active_list,
                               &seekbuf_tx->list_entry);
...
true_if_eq_sockbuf_id_cb(const struct picotm_slist* item, void* data)
{
    const struct sockbuf_tx* sockbuf_tx =
        sockbuf_tx_of_list_entry((struct picotm_slist*)item);
    assert(sockbuf_tx);
    assert(sockbuf_tx->sockbuf);
...
static struct sockbuf_tx*
get_sockbuf_tx(struct fildes_tx* self, const struct filebuf_id* id,
               struct picotm_error* error)
{
    /* Let's see if we already have the sockbuf in use.*/

    struct picotm_slist* pos = picotm_slist_find_1(&self->pipebuf_tx_active_list,
                                                   true_if_eq_sockbuf_id_cb,
                                                   (void*)id);
    if (pos != picotm_slist_end(&self->sockbuf_tx_active_list)) {
        struct sockbuf_tx* sockbuf_tx = sockbuf_tx_of_list_entry(pos);
        return sockbuf_tx;
    }
    ...
    pos = picotm_slist_front(&self->sockbuf_tx_alloced_list);
    if (pos) {
        picotm_slist_dequeue_front(&self->sockbuf_tx_alloced_list);
        return sockbuf_tx_of_list_entry(pos);
    }

    struct sockbuf_tx* sockbuf_tx = sockbuf_tx_create(error);
    if (picotm_error_is_set(error)) {
        return nullptr;
    }
                        ...
                        struct picotm_error* error)
{
    ...
    struct sockbuf_tx* sockbuf_tx = get_sockbuf_tx(self, &id, error);
    if (picotm_error_is_set(error)) {
        goto err_get_sockbuf_tx;
    }

    if (sockbuf_tx_holds_ref(sockbuf_tx)) {
        return sockbuf_tx; /* fast path: already holds a reference */
    }
    ...
    sockbuf_tx_ref_or_set_up(sockbuf_tx, sockbuf, error);
    if (picotm_error_is_set(error)) {
        goto err_sockbuf_tx_ref_or_set_up;
    }

    picotm_slist_enqueue_front(&self->sockbuf_tx_active_list,
                               &sockbuf_tx->list_entry);
                       ...
                       struct picotm_error* error)
{
    ...
    file_tx_ref_or_set_up(&chrdev_tx->base, &chrdev->base, nullptr, error);
    if (picotm_error_is_set(error)) {
                     ...
                     struct picotm_error* error)
{
    ...
    file_tx_ref_or_set_up(&fifo_tx->base, &fifo->base, nullptr, error);
    if (picotm_error_is_set(error)) {
        goto err_file_tx_ref_or_set_up;
    }
                        ...
                        struct picotm_error* error)
{
    ...
    file_tx_ref_or_set_up(&regfile_tx->base, &regfile->base, nullptr, error);
    if (picotm_error_is_set(error)) {
        goto err_file_tx_ref_or_set_up;
    }
    regfile_tx->seekbuf_tx = seekbuf_tx;
                    ...
                    struct picotm_error* error)
{
    ...
    file_tx_ref_or_set_up(&dir_tx->base, &dir->base, nullptr, error);
    if (picotm_error_is_set(error)) {
        goto err_file_tx_ref_or_set_up;
    }
                       ...
                       struct picotm_error* error)
{
    ...
    file_tx_ref_or_set_up(&socket_tx->base, &socket->base, nullptr, error);
    if (picotm_error_is_set(error)) {
        goto err_file_tx_ref_or_set_up;
    }
    socket_tx->sockbuf_tx = sockbuf_tx;
...
fildes_path(int fildes, struct picotm_error* error)
{
    ...
    char* path = realpath(symlink, nullptr);
    if (!path) {
        picotm_error_set_errno(error, errno);
        return nullptr;
    }
...
absolute_path(const char* path, struct picotm_error* error)
{
    ...
    char* cwd = cwd_module_getcwd(nullptr, 0, error);
    if (picotm_error_is_set(error)) {
        return nullptr;
    }
...
fildes_tx_exec_chmod(struct fildes_tx* self, const char* path, mode_t mode,
                     struct picotm_error* error)
{
    char* real_path = cwd_module_realpath(path, nullptr, error);
    if (picotm_error_is_set(error)) {
        return -1;
    }
...
fildes_tx_exec_link(struct fildes_tx* self, const char* path1, const char* path2,
                    struct picotm_error* error)
{
    char* real_path1 = cwd_module_realpath(path1, nullptr, error);
    if (picotm_error_is_set(error)) {
        return -1;
    }
...
static void
finish_sockbuf_tx(struct sockbuf_tx* sockbuf_tx, struct fildes_tx* fildes_tx)
{
    sockbuf_tx_finish(sockbuf_tx);
    sockbuf_tx_unref(sockbuf_tx);

    picotm_slist_enqueue_front(&fildes_tx->sockbuf_tx_alloced_list,
                               &sockbuf_tx->list_entry);
}

static void
finish_sockbuf_tx_cb(struct picotm_slist* item, void* data)
{
    finish_sockbuf_tx(sockbuf_tx_of_list_entry(item), data);
}

static void
finish_seekbuf_tx(struct seekbuf_tx* seekbuf_tx, struct fildes_tx* fildes_tx)
{
    seekbuf_tx_finish(seekbuf_tx);
    seekbuf_tx_unref(seekbuf_tx);

    picotm_slist_enqueue_front(&fildes_tx->seekbuf_tx_alloced_list,
                               &seekbuf_tx->list_entry);
}

static void
finish_seekbuf_tx_cb(struct picotm_slist* item, void* data)
{
    finish_seekbuf_tx(seekbuf_tx_of_list_entry(item), data);
}
...
{
    /* Unref socket buffers */
    picotm_slist_cleanup_1(&self->sockbuf_tx_active_list,
                           finish_sockbuf_tx_cb, self);
